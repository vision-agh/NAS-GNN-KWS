...
module convolution_ok #(
    ...
    parameter int ZERO_POINT_OUT             = 154, //good
    parameter int MULTIPLIER_OUT             = 58670, //good
    parameter int ZERO_POINT_WEIGHT          = 30075,
    parameter string INIT_PATH               = "???",
    parameter logic [PRECISION_IN-1:0] SCALE_IN [10:0]   = {0, 20, 40, 59, 79, 198, 178, 158, 139,119,99}
)(
    input logic clk,
    input logic reset,
    ...
    logic [$clog2(F_RADIUS):0]     iter_counter, iter_counter_MRR, iter_counter_RM2, iter_counter_RM, iter_counter_acc;
    logic [$clog2(OUTPUT_DIM)-1:0] outdim_counter, outdim_counter_mul_out, outdim_counter_compare, outdim_counter_acc;
    ...
    logic ena, ena_RM, web, enb, enb_RM, enb_RM2, ena_RM2;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            iter_counter_RM2 <= iter_counter_MRR;
            iter_counter_RM <= iter_counter_RM2;
            ena_RM2 <= ena;
            enb_RM2 <= enb;
            ena_RM <= ena_RM2;
            enb_RM <= enb_RM2;
    ...
    logic [$clog2(MAX_EDGES) :0] idx_a, idx_a_RM, idx_a_RM_reg;
    logic [$clog2(MAX_EDGES) :0] idx_b, idx_b_RM, idx_b_RM_reg;
    ...
    always @(posedge clk) begin
        idx_a_RM <= idx_a;
        idx_b_RM <= idx_b;
        idx_a_RM_reg <= idx_a_RM;
        idx_b_RM_reg <= idx_b_RM;
    end
    ...
    // Changed for -1
    always @(posedge clk) begin
        dt_scaled_a <= dt_expanded_a * MULTIPLIER_DIFF_T;
        dt_scaled_b <= dt_expanded_b * MULTIPLIER_DIFF_T;
        features_a[3] <= (iter_counter_RM == 0) ? ZERO_POINT_IN : (ena_RM ? ((dt_scaled_a>>>32)+dt_scaled_a[31]+ZERO_POINT_IN) : '0);
        features_a[4] <= (iter_counter_RM == 0) ? ZERO_POINT_IN : (ena_RM ? (SCALE_IN[edges_reg[idx_a_RM_reg].df]) : '0);
        features_b[3] <= enb_RM ? (dt_scaled_b>>>32)+dt_scaled_b[31]+ZERO_POINT_IN  : '0; //dif_t
        features_b[4] <= enb_RM ? SCALE_IN[edges_reg[idx_b_RM_reg].df] : '0;
    end
